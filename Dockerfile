...
ENV VIRTUAL_ENV /opt/Curso_FastAPI_Template_Backend-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/Curso_FastAPI_Template_Backend/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/Curso_FastAPI_Template_Backend/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/curso_fastapi_template_backend/ && touch src/curso_fastapi_template_backend/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/Curso_FastAPI_Template_Backend/
...
# Expose the app.
ENTRYPOINT ["/opt/Curso_FastAPI_Template_Backend-env/bin/poe"]
CMD ["api"]
